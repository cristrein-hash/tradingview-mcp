#!/usr/bin/env python3
"""JACKKNIFE por EPISÓDIO de BULL/RANGE (não por ano) sobre a base #4 final (regime v5 hour-causal).
Episódios = runs contíguos não-BEAR do regime v5. Para cada: contribuição própria + base SEM ele.
Responde: o edge se repete entre tendências de alta, ou é 1 episódio? Causal, determinístico."""
...
for j in range(len(HK)):
    ...
    ov_hour.append(ov)
def regime_hc(cjt):
    di=bisect.bisect_left(DK,cjt//86400)-1
    s="RANGE" if di<0 else stable[di]
    hi=bisect.bisect_right(HK,(cjt//3600)-1)-1
    ovr= ov_hour[hi] if hi>=0 else False
    return "BEAR" if (ovr or s=="BEAR") else s
# regime v5 diário (p/ segmentar episódios)
trig=set()
for j in range(max(P,mom),len(HK)):
    peak=max(HH[j-P:j+1]); ddp=(peak-HC[j])/peak if peak>0 else 0
    if ddp>=dd_intra and HC[j]<HC[j-mom]: trig.add(HK[j])
dset=set(hk*3600//86400 for hk in trig)
...
# episódios = runs contíguos não-BEAR
EP=[]
for i in range(len(DK)):
    if v5d[i]=="BEAR": continue
    if EP and DK[i]==EP[-1][2]+1: EP[-1][2]=DK[i]
    else: EP.append([v5d[i],DK[i],DK[i]])  # label-inicial, start_dk, end_dk
def epname(e):
    return f"{dt.datetime.utcfromtimestamp(e[1]*86400).strftime('%y-%m-%d')}..{dt.datetime.utcfromtimestamp(e[2]*86400).strftime('%y-%m-%d')}"
def ep_of(dk):
    for ix,e in enumerate(EP):
        if e[1]<=dk<=e[2]: return ix
    return -1
HMAX=480; RCAP=20.0
def cf_low(s,i):
...
for r in ROWS:
    ...
    if not atr: continue
    if regime_hc(r["cj_t"])=="BEAR": continue
    ...
    base.append({"cj_t":r["cj_t"],"R":R,"ep":ep_of(r["cj_t"]//86400)})
def metr(rows):
    R=[x["R"] for x in sorted(rows,key=lambda z:z["cj_t"])]; n=len(R)
    if not n: return (0,0,0,0)
    sm=sum(R); eq=pk=dd=0
    for x in R: eq+=x; pk=max(pk,eq); dd=min(dd,eq-pk)
    return n,round(sm,1),round(sm/n,3),round(dd,1)
n0,sm0,av0,dd0=metr(base)
print(f"BASE #4 final (regime v5 hour-causal): N{n0} sumR{sm0} avgR{av0} DD{dd0}\n")
# episódios com >=1 trade
used=sorted(set(c["ep"] for c in base if c["ep"]>=0))
print("EPISÓDIOS operáveis (com trades) — contribuição própria:")
print(f"{'episódio':<22}{'lab':<6}{'N':>4}{'sumR':>8}{'avgR':>7}")
rows=[]
for ix in used:
    e=EP[ix]; tr=[c for c in base if c["ep"]==ix]; n,sm,av,dd=metr(tr)
    rows.append((sm,ix,e,n,av)); print(f"{epname(e):<22}{e[0]:<6}{n:>4}{sm:>8}{av:>7}")
print(f"\nJACKKNIFE — base SEM cada episódio:")
print(f"{'removendo':<22}{'N':>5}{'sumR':>8}{'avgR':>7}{'DD':>7}")
for sm,ix,e,n,av in sorted(rows,reverse=True):
    rest=[c for c in base if c["ep"]!=ix]; rn,rsm,rav,rdd=metr(rest)
    print(f"{epname(e):<22}{rn:>5}{rsm:>8}{rav:>7}{rdd:>7}  (tirou N{n} {sm}R)")
avgs=[av for sm,ix,e,n,av in rows]
print(f"\navgR por episódio: min {min(avgs):.3f} / mediana {st.median(avgs):.3f} / max {max(avgs):.3f}  (consistência entre bulls)")
