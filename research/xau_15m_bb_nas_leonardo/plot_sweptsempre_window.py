...
for r in ROWS:
    ...
    r["_R"]=R; r["_entry"]=entry; r["_sl"]=sl; r["_atr"]=atr; r["_sw"]=f(r,"swept_prior_low",0)==1
    base.append(r)
...
SW=[]
for c in clusters:
    if len(c)==1: SW+=c; continue
    sw=[t for t in c if t["_sw"]]; SW += (sw if sw else c)
LO=dt.datetime(2025,8,1).timestamp()
sel=[t for t in base if t["_sw"] and t["cj_t"]>=LO]
sel.sort(key=lambda z:z["cj_t"])
out=[]
for i,t in enumerate(sel,1):
    entry=t["_entry"]; sl=t["_sl"]; R=t["_R"]; ex=entry+R*(entry-sl)
    out.append({"num":i,"cj_t":t["cj_t"],"date":dt.datetime.utcfromtimestamp(t["cj_t"]).strftime("%Y-%m-%d %H:%M"),
                "entry":round(entry,2),"sl":round(sl,2),"exit":round(ex,2),"R":round(R,3),"win":R>0})
with open(HERE/"sweptsempre_window.csv","w",newline="") as fh:
...
print(f"swept-sempre de 2025-08-01: N={len(out)} WR={100*wn/len(out):.1f}% sumR={sm:.1f} | {out[0]['date']} -> {out[-1]['date']}")
# plot
