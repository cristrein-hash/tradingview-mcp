...
for r in ROWS:
    ...
    lo20=min(x["l"] for x in s[max(0,cj-19):cj+1]); hi20=max(x["h"] for x in s[max(0,cj-19):cj+1]); chc=1 if (f(r,"h4n_choch_up_rec",0)>=1 or f(r,"h1n_choch_up_rec",0)>=1) else 0
    pos20=(entry-lo20)/((hi20-lo20) or atr); rsicj=s[cj].get("rsi") or 50
    B.append({"cj_t":r["cj_t"],"entry":entry,"sl":sl,"R":R,"pos20":pos20,"rsi":rsicj,"chc":chc})
...
S4all=[x for x in B if x["pos20"]>=qpos and x["rsi"]>=qrsi]; S4=[x for x in S4all if x["chc"]==1]
...
with open(HERE/"choch_window.csv","w",newline="") as fh:
