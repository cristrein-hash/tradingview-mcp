#!/usr/bin/env python3
"""Plotagem CANONICA dos 170 trades: A2 5ATR + h1_eff>=0.15 + 4H macro!=BEAR + B v3 diario!=BEAR (Cris escolheu 2026-06-27).
long_position entry/SL(A)/saida-letrun + label #N VERDE(win)/VERMELHO(loss). Requer pause flag. NAO screenshot, NAO draw_clear."""
import sys,csv,json
...
def main():
    ...
    rows=list(csv.DictReader(open(HERE/"strategy_5atr_regime170_trades.csv"))); rows.sort(key=lambda r:int(r["entry_t"]))
    ...
    try:
        st=c.call_tool("chart_get_state"); chart["before"]={"symbol":st.get("symbol"),"tf":str(st.get("resolution")),"draw_list":None}
        ...
        if not (str(sym).endswith("XAUUSD") and res==TF):
            c.stop(); print(json.dumps({"HARD_STOP":f"chart nao 15: {sym}/{res}"})); return 1
        dl0=c.call_tool("draw_list"); chart["before"]["draw_list"]=dl0.get("count") if isinstance(dl0,dict) else None
        ...
        dl=c.call_tool("draw_list"); chart["draw_list_count_after"]=dl.get("count") if isinstance(dl,dict) else None
    finally:
        try: c.stop()
