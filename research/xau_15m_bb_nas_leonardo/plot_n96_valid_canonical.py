#!/usr/bin/env python3
"""Plotagem CANONICA dos 83 trades VALIDOS do N96 entry engine (fonte n96_valid_trades.csv = 96 - 13 cortados
intra-BEAR). Copia fiel de plot_chosen_canonical.py (plotting-canon §7/§8): long_position entry/SL/saida-real +
label #num VERDE(winner)/VERMELHO(loser), width 10 (15M), tick offsets. Requer pause flag. NAO screenshot, NAO
draw_clear (Cris limpa manual). Verificacao = success + draw_list."""
import sys,csv,json
...
SYMBOL,TF,BAR_S,WIDTH="PEPPERSTONE:XAUUSD","15",900,10  # canon 15M=10
GREEN,RED="#1a8917","#cc0000"
def main():
    if not PAUSE.exists(): print("ERRO: pause flag ausente"); return 1
    rows=list(csv.DictReader(open(HERE/"n96_valid_trades.csv"))); rows.sort(key=lambda r:int(r["entry_t"]))
