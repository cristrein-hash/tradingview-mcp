#!/usr/bin/env python3
"""Re-roda SUBSTRATO #4 trocando o gate de regime v2 -> v5 (MTF: estável diário + override 1H drawdown%).
Thresholds fixos (pos20>=0.346, rsi>=45.5) p/ isolar o efeito do regime. Compara painel #4(v2) vs #4(v5) +
lista os trades que o v5 corta (early-BEAR). Determinístico, causal."""
...
def hyster(rawl,K,Kbear):
    ...
    for v in rawl:
        ...
        out.append(cur)
    return out
stable=hyster(rawS,K,Kbear)
DAYREG_V2={DK[i]:stable[i] for i in range(len(DK))}   # v2 = camada estável (mesma do build #4 original)
# override 1H -> v5
P,mom,dd_intra,Krec=48,24,0.06,5
trig_h=set()
...
for i in range(len(DK)):
    ...
    v5.append("BEAR" if (ov or stable[i]=="BEAR") else stable[i])
DAYREG_V5={DK[i]:v5[i] for i in range(len(DK))}
# ---- substrato #4 pipeline ----
HMAX=480; RCAP=20.0
def cf_low(s,i):
...
for r in ROWS:
    ...
    dk=r["cj_t"]//86400
    cand.append({"cj_t":r["cj_t"],"yr":r["yr"],"R":R,"v2":DAYREG_V2.get(dk,"RANGE"),"v5":DAYREG_V5.get(dk,"RANGE")})
def panel(rows,tag):
    ...
    print(f"{tag:<26} N{n:>4} WR{100*w/n:>5.1f}% W{w}/L{n-w} run{sum(1 for x in R if x>=3)} | sumR{sm:>7.1f} avgR{sm/n:>6.3f} DD{dd:>6.1f} r/DD{abs(sm/dd) if dd<0 else 99:>5.2f} streak-{mL}/+{mW} | yr {py[2024]}/{py[2025]}/{py[2026]}")
S4v2=[c for c in cand if c["v2"]!="BEAR"]
S4v5=[c for c in cand if c["v5"]!="BEAR"]
print("SUBSTRATO #4 — gate de regime v2 vs v5:\n")
panel(S4v2,"#4 regime v2 (atual)")
panel(S4v5,"#4 regime v5 (MTF)")
cut=[c for c in S4v2 if c["v5"]=="BEAR"]
cut.sort(key=lambda z:z["cj_t"])
print(f"\nv5 corta {len(cut)} trades que v2 mantinha (early-BEAR): losers {sum(1 for c in cut if c['R']<=0)} winners {sum(1 for c in cut if c['R']>0)} sumR {sum(c['R'] for c in cut):.1f}")
for c in cut: print(f"  {dt.datetime.utcfromtimestamp(c['cj_t']).strftime('%Y-%m-%d %H:%M')}  R {c['R']:+.2f}")
