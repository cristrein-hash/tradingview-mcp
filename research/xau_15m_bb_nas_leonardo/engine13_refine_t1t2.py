#!/usr/bin/env python3
"""ENGINE 13 (Cris 2026-06-28): refino contextual T1 (de-interpolar) + T2 (anti-topo-range) sobre a BASE FIXA
(3120 + h4_up&h1d_up, regime BULL/RANGE). Causal. Reporta BASE / +T1 / +T2 / +T1+T2 na base inteira e na janela
ago2025->jan2026, painel completo (incl streak) + por-ano + sensibilidade G/thr."""
...
LDON=96  # Donchian intraday p/ range ativo (1 dia 15m)
base=[]
for r in ROWS:
...
print(f"BASE FIXA={len(base)}  (LDON={LDON})")
def T2(rows,thr):  # anti-topo-range: em RANGE, exige metade inferior
    return [r for r in rows if not (r["reg"]=="RANGE" and r["rpos"]>thr)]
def T1(rows,G,D):  # de-interpolar: suprime reentrada mesmo nivel dentro de G barras
    taken=[]; out=[]
    for r in rows:
...
def metr(rows):
    ...
    return n,round(100*w/n,1),round(sm,1),round(sm/n,3),round(dd,1),round(rd,2),f"-{mL}/+{mW}",py
def show(tag,rows):
    m=metr(rows)
    if not m: print(f"{tag:<26} vazio"); return
    n,wr,sm,avg,dd,rd,sk,py=m
    print(f"{tag:<26} N{n:>4} WR{wr:>5}% sumR{sm:>7} avgR{avg:>6} DD{dd:>6} r/DD{rd:>5} streak{sk:>7}  yr {py[2024]}/{py[2025]}/{py[2026]}")
LO=dt.datetime(2025,8,1).timestamp(); HI=dt.datetime(2026,1,1).timestamp()
def win(rows): return [r for r in rows if LO<=r["cj_t"]<HI]
G,D,THR=12,0.6,0.5
print(f"\n### CONFIG PRINCIPAL: T1(G={G}b,D={D}ATR) | T2(thr={THR}, metade inferior do range) ###")
for scope,sel in (("— BASE INTEIRA (2024-26) —",base),("— JANELA ago2025->jan2026 —",win(base))):
    print(scope)
    show("BASE",sel)
    show("+T1 (de-interpola)",T1(sel,G,D))
    show("+T2 (anti-topo-range)",T2(sel,THR))
    show("+T1+T2",T1(T2(sel,THR),G,D))
print("\n### SENSIBILIDADE (base inteira, T1+T2) ###")
for G2 in (8,12,16):
    for THR2 in (0.4,0.5,0.6):
        show(f"G={G2} thr={THR2}",T1(T2(base,THR2),G2,D))
