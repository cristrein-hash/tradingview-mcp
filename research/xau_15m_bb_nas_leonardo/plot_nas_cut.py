...
for r in ROWS:
    ...
    B.append({"cj_t":r["cj_t"],"entry":entry,"sl":sl,"R":R,"pos20":pos20,"rsi":rsicj,"nas":f(r,"nas_long_16",0)})
...
S4all=[x for x in B if x["pos20"]>=qpos and x["rsi"]>=qrsi]; S4=[x for x in S4all if x["nas"]>=1]
...
with open(HERE/"nas_cut_window.csv","w",newline="") as fh:
