...
for r in ROWS:
    ...
    lo20=min(x["l"] for x in s[max(0,cj-19):cj+1]); hi20=max(x["h"] for x in s[max(0,cj-19):cj+1]); flow=1 if (f(r,"nas_long_16",0)>=1 or f(r,"sell_bub_w",0)>=2 or f(r,"in_demand",0)==1) else 0
    pos20=(entry-lo20)/((hi20-lo20) or atr); rsicj=s[cj].get("rsi") or 50
    B.append({"cj_t":r["cj_t"],"entry":entry,"sl":sl,"R":R,"pos20":pos20,"rsi":rsicj,"flow":flow})
...
S4=[x for x in B if x["pos20"]>=qpos and x["rsi"]>=qrsi and x["flow"]==1]
...
with open(HERE/"flow_tagged_window.csv","w",newline="") as fh:
...
print(f"FLOW-TAGGED (nas_long|sell_bub>=2|in_demand) substrato#4 (qpos={qpos:.3f},qrsi={qrsi:.1f}) janela ago2025->29jan2026: N={len(out)} WR={100*wn/len(out):.1f}% sumR={sm:.1f} | full-S4 N={len(S4)}")
