#!/usr/bin/env python3
"""LAYER 2 — IMPULSE-DIP v2: perna como IMPULSO, dip PROPORCIONAL (2026-07-05).
...
def upleg_feats(u):
    ...
    hi_k = max(range(i - 48, i + 1), key=lambda k: S[k]["h"]); hi = S[hi_k]["h"]
    lo_k = min(range(max(0, hi_k - 96), hi_k + 1), key=lambda k: S[k]["l"]); lo = S[lo_k]["l"]
...
json.dump({"universe": {"n": len(L2U), "hit": round(uni_hit, 3)},
          ...
          open(HERE / "results" / "layer2_impulsedip_v2_20260705.json", "w"), indent=1)
print("OK → results/layer2_impulsedip_v2_20260705.json")
