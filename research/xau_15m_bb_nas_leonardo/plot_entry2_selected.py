...
def main():
    ...
    rows=list(csv.DictReader(open(HERE/"entry2_selected_trades.csv"))); rows.sort(key=lambda r:int(r["cj_t"]))
    ...
    try:
        ...
        for r in rows:
            entry=float(r["entry"]); sl=float(r["sl"]); ex=float(r["exit"]); t=int(r["cj_t"]); win=r["win"]=="1"; num=r["num"]
            ...
            else: fails.append({"num":num,"err":str(r1)[:100]})
