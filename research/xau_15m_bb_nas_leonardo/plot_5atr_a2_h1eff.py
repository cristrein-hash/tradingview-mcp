#!/usr/bin/env python3
"""Plotagem CANONICA dos 211 trades A2 5ATR + filtro h1_eff>=0.15 (APROVADO Cris 2026-06-27):
long_position entry/SL(A flush-0.1ATR)/saida-real-letrun + label #N VERDE(winner)/VERMELHO(loser).
Requer pause flag. NAO screenshot, NAO draw_clear (Cris ja limpou)."""
import sys,csv,json
...
def main():
    ...
    rows=list(csv.DictReader(open(HERE/"strategy_5atr_a2_h1eff_trades.csv"))); rows.sort(key=lambda r:int(r["entry_t"]))
