#!/usr/bin/env python3
"""Limpa o chart e plota APENAS os 55 fundos BULL not_clean NOVOS (não-interpolados com absorb) — long_position +
label azul. Visão limpa. Régua oficial (SL demanda −0.1ATR, +3R, 20b). Verified 2026-06-25."""
...
new = [i for i in bull if any(lo >= C[i] for hi, lo in asof(TS[i]).get("zones", [])) and i not in absorb]
print(f"55 novos esperados; calculados = {len(new)}")
...
print(f"VISÃO LIMPA: {lp} long_position + {lp} labels azuis (55 novos not_clean); chart NAO restaurado")
cli.stop()
