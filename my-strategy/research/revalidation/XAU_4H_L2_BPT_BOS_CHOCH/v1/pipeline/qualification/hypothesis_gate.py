...
def cmd_sanity():
    ...
    add("capit_rsi_registered", bool(capr) and capr["status"] in ("PROMISING_IN_SAMPLE", "OOS_CANDIDATE", "CONTEXT_ONLY") and capr["allowed_engine_use"] in ("REVIEW_ONLY", "CONTEXT_ONLY"), f"status={capr and capr['status']} use={capr and capr['allowed_engine_use']}")
    gate_rows = cmd_dry_run()
    cr = next((r for r in gate_rows if r["hypothesis_id"] == (capr or {}).get("hypothesis_id")), {})
    add("validation_dry_run_pass", cr.get("validation_ready") == "YES" or (capr or {}).get("status") in ("CONTEXT_ONLY", "REJECTED", "RETIRED", "VETO_ONLY"), f"ready={cr.get('validation_ready')} (terminal status {(capr or {}).get('status')} = OK)")
