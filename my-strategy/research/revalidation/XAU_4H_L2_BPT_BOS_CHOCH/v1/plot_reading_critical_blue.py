#!/usr/bin/env python3
"""Plota os 42 episódios CRÍTICOS da leitura (32 críticos ∪ 13 do loser-streak) — plotagem CANÔNICA SIMPLES
(long_position + label #id), labels AZUIS (#2962ff) p/ diferenciar da plotagem anterior que fica no chart.
- ADITIVO: NÃO chama draw_clear, NÃO troca symbol/timeframe, NÃO captura screenshot.
- Exit canônico p/ geometria visual: stop −1.0 ATR / target +2.7 ATR (todos LONG).
- Segura o chart_op.lock do daemon L1 durante a sessão MCP (daemon aborta se lock fresco); remove no finally.
- HARD STOP se chart não for PEPPERSTONE:XAUUSD/240 ou algum trade inválido.
Convenção idêntica a plot_t8_canonical.py / docs/CANONICAL_TRADE_PLOTTING.md (só a cor do label muda p/ azul)."""
import json, math, subprocess, sys, time, csv
...
for d in (V1, *V1.parents):
    ...
    if (d / "my-strategy").is_dir() and (d / "alert-bridge").is_dir():
        ROOT = d
        if (d / "src" / "server.js").exists(): break
...
RAW = V1 / "results/.." / "repro_recovery/raw_features_2020_2026.jsonl"
RAW = V1 / "repro_recovery/raw_features_2020_2026.jsonl"
PLOT_CSV = V1 / "results/l2_bpt_episode_reading_plot_list_v2.csv"
HORIZON_S = 40 * 4 * 3600   # largura da caixa: 40 barras 4H
...
class MCPClient:
    ...
    def start(self):
        ...
        r=self._raw("initialize",{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"plot-reading","version":"1.0"}})
...
def build_trades():
    ...
    rows=list(csv.DictReader(open(PLOT_CSV)))
    trades=[]
    for r in rows:
        b=int(r['bar_idx']); atr=ATR[b]
        ...
        trades.append(dict(id=b,entry_price=round(entry,2),stop_price=round(stop,2),target_price=round(tgt,2),
            entry_time=TS[b],exit_time=TS[b]+HORIZON_S,color=BLUE,reason=r['reason']))
    return trades
...
def main():
    ...
    print(f"trades válidos: {len(trades)} | labels AZUIS {BLUE} | aditivo (sem clear)")
    CHART_LOCK.parent.mkdir(parents=True,exist_ok=True)
    CHART_LOCK.write_text(f"plot_reading_critical_blue pid={__import__('os').getpid()} {time.time()}\n")
    ...
    try:
        ...
        for k,t in enumerate(trades):
            ...
            if (k+1)%15==0: print(f"  [{k+1}/{len(trades)}] pos={ok_pos} lbl={ok_lbl} fail={fail}")
        ...
        print(json.dumps({"trades":len(trades),"long_position_ok":ok_pos,"label_ok":ok_lbl,
            "failures":fail,"expected_shapes":2*len(trades),"draw_list_total":n},indent=2))
    finally:
        client.stop()
        try: CHART_LOCK.unlink()
        except FileNotFoundError: pass
        print("chart_op.lock removido (ciclo 03:05 roda normal).")
    return 0
