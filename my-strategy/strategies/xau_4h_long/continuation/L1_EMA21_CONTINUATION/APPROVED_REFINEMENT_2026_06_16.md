...
- **`runtime_xau.py` (LIVE): HARD STOP no filtro NAS (regra #10).** O snapshot MCP live só dá study_value corrente; **NAS_DISTANCE no bar i-1 NÃO é extraível causalmente** (sem histórico per-bar de study-value; recomputar = aproximar = proibido). Logo o runtime live **NÃO** emite operational sob a config completa — segue não-operacional (também depende de base-rule live). **scanner = gate autoritativo até o NAS-live ser resolvido** (bloco futuro: persistir NAS por ciclo em .runtime_state OU tool MCP de histórico de study-value).
