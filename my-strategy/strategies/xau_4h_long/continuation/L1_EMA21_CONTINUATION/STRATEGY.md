...
3. **✅ REGIME SPLIT-BRAIN RESOLVIDO (2026-06-16).** O `scanner.py` foi **unificado** com o `runtime_xau.py`: ambos agora usam **`regime_l1_v4`** (mesmo arquivo + mesma função `latest_state_before`). `regime_B_v3` **não entra mais** no caminho L1. **Re-derivação sob o regime live** (research/in-sample, `NOT_VALIDATION` — ver `research/revalidation/.../rederive_regime_l1v4/`): os 38 candidatos antigos são **38/38 preservados, 0 removidos**, MAS aparecem **+25 candidatos NOVOS (63 no total)** que o `regime_B_v3` bloqueava. **Consequência: os números antigos do set de 38 NÃO representam o regime live** (há 25 bars de outcome desconhecido) → continuam research/in-sample até RAW OOS. #1/#11/#36/#38 seguem `operational_candidate`; #3/#15/#18/#32 seguem `blocked_exhaustion` (RSI gate, regime-independente).
...
- Regime macro **D-1 = BULL** (causal, SHIFT1) via **`regime_l1_v4`** — **unificado** entre `scanner.py` e `runtime_xau.py` (2026-06-16; ver banner). `regime_B_v3` não entra mais no caminho L1.
