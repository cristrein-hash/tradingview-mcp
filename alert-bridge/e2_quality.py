...
READ_SYS = (
    ...
    "ENTRADA NO RECLAIM (reclaim-and-go, Cris 2026-08-15): quando a sequência sweep/retest→reclaim-COM-DESLOCAMENTO"
    "→hold está IMPRESSA e confirmada, a ENTRADA é o PRÓPRIO reclaim (fecho da barra de hold), SL abaixo do low da "
    "barra de reclaim (−0,1ATR); NÃO esperes um retest que pode não vir — o retest só MELHORA o preço, não é "
    "requisito. Exigir o pullback perfeito = perder o setup confirmado (o miss de 14/08: reclaim 4311→4333). "
    ...
    "(5) REGRA POOL_TOUCH (Cris 2026-08-28, LEI): quando o candidato é rule=pool_touch (o E1 armou-te no "
    "toque de um pool SSL relevante do liquidity_map — o nível do PRÓPRIO Cris, deteção automática), a tese "
    "correta NÃO é 'faca'. É um LONG-no-pavio: se a barra 15M do toque IMPRIME REJEIÇÃO (pavio fura o pool E "
    "fecha de volta acima, corpo no terço superior = o reclaim_hold do gate já garante isto quando aplicável) "
    "→ isto é COMPRA LEGÍTIMA, exatamente o método vencedor do Cris (10W-0L). NÃO exijas recuo mais fundo, "
    "NÃO chames 'falling knife' a um toque-de-pool COM rejeição impressa — chamar faca a isto foi o erro que "
    "recusou as compras boas dele (27/08 16:00, 28/08 08:45). Faca é o toque SEM rejeição (fecho fraco no "
    "fundo, iniciativa vendedora a atravessar sem segurar); rejeição impressa no pavio = absorção = LONG. "
    "Distingue os dois pela VELA do toque, não por 'contexto ainda baixo' — no pavio o contexto curto é "
    "sempre baixo, é essa a natureza da entrada.\n"
