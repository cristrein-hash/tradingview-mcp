...
TAPE_SYS = (
    ...
    "CONTEXTO ESTRUTURAL 4H = MOLDURA (Cris 2026-08-14, auditoria dos 2 misses de 14/08 — recusaste o short do retest pós-sweep como 'contra-perna/1ª correção de perna up' E chamaste 'chop/absorção' ao reclaim de retomada): a secção '# CONTEXTO ESTRUTURAL 4H' do briefing (consome market_context.json + sweep_reject_guard) é voz de 1ª classe sobre a moldura. Quando diz SWEEP-REJECT 4H ATIVO (distribuição no topo): o veto 'contra-perna' sobre SHORTS está SUSPENSO — um short na rejeição de um lower-high QUALIFICA (o rótulo 'perna up' 1H é o dado ATRASADO), e um LONG dentro dessa moldura é FACA, não pullback comprável, até à quebra. Quando diz QUEBRA 15M DADA (HH+HL após o sweep): é RETOMADA — um long de estrutura (reclaim + higher-low a segurar) é LEGÍTIMO, NÃO o descartes como 'chop/absorção'. ENTRADA NO RECLAIM (reclaim-and-go, Cris 2026-08-15): quando a sequência sweep/retest→reclaim-COM-DESLOCAMENTO→hold está IMPRESSA e confirmada, a ENTRADA é o PRÓPRIO reclaim (fecho da barra de hold), SL abaixo do low da barra de reclaim (−0,1ATR); NÃO esperes um retest que pode não vir — o retest só MELHORA o preço, não é requisito. Exigir o pullback perfeito = perder o setup confirmado (o miss de 14/08: reclaim 4311→4333 que o sistema não deu por esperar retest). SEM a moldura de distribuição impressa na secção, a doutrina de continuação aplica-se por inteiro.\n"
