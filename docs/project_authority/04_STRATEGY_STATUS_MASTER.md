...
| `L2/BPT XAU 4H LONG · trend-exit / regime-flip` | 4H | `LIVE_PRODUCTION_ALERT_ONLY` (go-live Cris 2026-07-17) | **LIVE**: runtime próprio `L2_BPT_ZONE_TREND_EXIT/` + LaunchAgent `com.cristrein.xau-l2-cycle` (6×/dia :12 Lisboa, tab-pinned 4H) + Telegram ativo (`L2_PRODUCTION_AUTHORIZED=1`, allowlist XAU_240). Alert-only, zero auto-trade. Gates cumpridos: paridades V-1..V-4 byte-exatas (17 trades + painel +105.3R/DD−4.1/streak3 + régua 245 re-derivada) + DA causalidade runtime PASS + dry-run launchd + teste Telegram confirmado por Cris. | **Estratégia oficial aprovada (Cris 2026-07-08); EM PRODUÇÃO desde 2026-07-17.** L2/BPT + novo exit trend-exit (segura enquanto regime/tendência persiste, sai na virada). SELECT-17 +105.3R (retDD 26×, streak3) vs +36.2R let-run; FULL ~+385.7R. Causalidade DA=PASS (não look-ahead). Caveats aceitos: ~78% do ganho=horizonte/exposição; full-base DD−72/streak22 (produção exige camada exec/risco). #6 mecânico +1.15R. See §4.4 + `L2_BPT_TREND_EXIT_OFFICIAL_APPROVAL_20260708.md`. |
...
- **UPDATE 2026-07-17 (go-live autorizado por Cris):** `xau-l1-cycle` (L1 EMA21 4H) e `xau-l2-cycle` (L2/BPT trend-exit) estão **LIVE** — LaunchAgents carregados, Telegram ativo, alert-only (zero auto-trade). Modo tab-pinned (tabs dedicadas 5M/15M/1H/4H/1D via `TVMCP_TARGET_CHART_ID`; helper `my-strategy/core/tab_pin.py`); requisito operacional: manter as 5 tabs XAUUSD abertas com os estudos visíveis (NAS + OB boxes + Bubbles + RSI no 4H). O texto DORMANT abaixo descreve o estado pré-2026-07-17 (histórico).
- (histórico pré-go-live) 4H strategy layer = DORMANT/SUPERSEDED: `xau-l1-cycle` PAUSED, `monitor_xau_4h_strategies.py` not running. Older "core system (2026-06-03)" superseded; V1.4g-RWS-A6-A7, Caminho B LONG, Regime Classifier v3 = DORMANT until re-verified.
- **Legacy monitors** (`monitor_xau_4h_strategies.py`, cron) continuam DORMANT — não recarregar sem autorização + runbook.
