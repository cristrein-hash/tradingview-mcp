...
## LEITOR FRACTAL MTF (2026-07-07) — o fractal vindica-se; HTF-demand-retest = lead promissor causal
Cris (correto): a miopia era FRACTAL/single-scale; o `ms_state` 15M colapsou no calendário. Resample RAW 15M→4H/1D/1W (kit `mtf_kit.py`, anti-lookahead: barra HTF corrente EXCLUÍDA). Workflow wf_25035e50: 6 famílias HTF, portão OOF (leave-one-out) + mining-null. **5 falham OOF** (fase-4H 0,486, fase-1D 0,515, maturidade-aninhada 0,468, alinhamento 0,600/p0,155, posição/room). **1 SOBREVIVE: htf_demand_retest (retest de demanda HTF 4H/1D) — OOF 0,647, poison 0,73, causal 0/96 violações, mining_null_p(marginal) 0,01.** **DA COMPOSITE MINING-NULL:** best-of-6 P=0,08; **best-of-13 (6 famílias+7 sínteses) P≈0,14 > 0,10** → como resultado SELECIONADO in-sample = winner's-curse de seleção. **MAS DA confirma o núcleo:** (a) CAUSAL-limpo (0 lookahead, 0 uso de outcome, barra HTF corrente excluída); (b) FRACTAL GENUÍNO não-proxy — ablação: só-demanda 0,667 vs só-maturidade/posição 0,585(base); reclaim_lag sozinho 0,527(<base), corr<0,14 → NÃO é reclaim-R/regime disfarçado; (c) LEVANTA AMBOS ANOS (DA corrigiu: base-2026=46% não 54%; 2025 +11,1pp, 2026 +8,2pp). Robusto ao threshold. **VEREDITO: (b) PROMISSOR-NÃO-VALIDADO — 1ª feature da sessão causal+não-proxy+ambos-anos; falha só na multiplicidade de seleção. CAMINHO (canon, sem OOS): pré-registar hipótese única = demand-only 8-feat, validar FORWARD/janela virgem (próximos trades live do Cris), onde p=0,01 de hipótese-única é honesto.** Scripts `mtf_feat_htf_demand_retest.py`, DA `_DA_composite_*.py`. Vindica a leitura fractal do Cris: a fase verdadeira lê-se por retest de demanda nas escalas HTF, não no 15M isolado.
