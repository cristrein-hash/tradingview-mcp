...
### Lab G — Entry contextual de capitulação 15M — ✅ EXECUTADO 2026-07-03 (rodada criativa pós-crítica Cris)
Auto-auditoria (indicadores RAW nunca definiam entrada; regime só gate; zero indicadores novos) → universo 4499×74 campos (13 indicadores NOVOS das séries RAW) → 3 designers+DA-pré+síntese → 2 sistemas congelados. **Sistema A "EMA-SHAKEOUT" (BULL-only): N53 WR_liq 60,4 · NET+25,9 · DD−3,2 · stk−3 · 0,9/sem-BULL → único desenho da campanha a passar o gate FN na estimativa pontual; POSITIVO_FRÁGIL (DA): causal limpo (1.856 checks), kill-null estável ≥90, MAS evidência ≈ nula após desconto familiar (~18 olhadas) e 88% do NET em 2025. 70% do NET em 21 trades FORA da base435 (flushes não-swept, WR76) = substrato novo real.** Sistema B PoT-Map FAIL FN (P7 78%/38%). Negativos valiosos: depth-first anti-seletivo · floor-raid RANGE 15M (veto permanente). Pendências bloqueantes de A: RAW estendido + visual Cris (21 novos) + N fora-2025. Docs: `XAU_15M_LONG_LAB_G_CONTEXTUAL_ENTRY_{OPENING,RESULTS}_20260703.md`.

### Lab B r2 — Structural context convergence — ✅ EXECUTADO 2026-07-04: **RISK_CONTROL_ONLY**
Discovery 4 perspectivas+DA-pré (~100 looks declarados = CALIBRAÇÃO) → 5 famílias congeladas → medição oficial → DA independente (adjudicou: feats do regime box CAUSAIS — o BLOCKED era sign-flip do assert; corrigido PASS 35/35). **Achado central = CANON NEGATIVO FB1: teto/supply convergente NÃO é tóxico na base — carrega PRÊMIO (runners moram no meio-alto/teto); SKIP por teto = taxar pagadores** (refuta a tese fundadora como corte; 4 buscas independentes). Losers reais moram no FUNDO/early-leg (FB2: legpos60≤0,25 & h1_pos≤0,61; 42 trades WR33 −6,0) → **FB2-SIZE50 via F4 floor 0,5 = único acionável: +236,6 NET (101,3%), DD −11%, 53/53 runners — risk-control aritmético, 2026 contradiz (flagged +4,4)**. FB3 limbo pós-breakout = prateleira (anulado por FB1, resíduo N4). WR/streak INALTERADOS — **medido: 57% dos trades da janela do max-DD estão dentro da classe protegida → DD/streak inatacáveis por contexto ex-ante**. Forward-ledger congelado (listas cj_t; N≥15 exige múltiplas extensões RAW). Docs `XAU_15M_LONG_LAB_B_R2_STRUCTURAL_CONTEXT_{DISCOVERY,PREREG,DA,REPORT}_20260704.md`.

### MTF Signature Gate Test — ✅ EXECUTADO 2026-07-04: **SIGNATURE_GATE_FAIL (assinatura REFUTADA como fill-fiction)**
A assinatura do discovery das 35 operações manuais (supply_far_3atr 15M & demand_near_1atr 1H; lift 5,7-6,7×) COLAPSOU no teste congelado de outcomes: E1 BASE∩GATE retém 1,9% do NET com runner-kill 52/53 e nulls ≈ aleatório; E2 standalone NET −13,6. **Mecanismo adjudicado pelo DA: o lift era o preço-âncora retroativo dos desenhos (~3,3 ATR abaixo do mercado) inflando as DUAS pernas — com preço real no MESMO instante, 6% ≈ controles 8%.** Os 21 fora-da-base do Sistema A: 0/21 pass (não é este o substrato deles). **Lição de método permanente: alvos de hindsight-target devem ser REPRECIFICADOS ao close real antes de qualquer lift; lentes preço-dependentes herdam o artefato.** Pistas preço-independentes (CHoCH ≤24b, absorção SELL-M/L, anti-iniciativa, NAS-recência, swept 43%) = não-refutadas, nunca validadas. Docs `XAU_15M_LONG_MTF_SIGNATURE_{DISCOVERY,GATE_PREREG,GATE_DA,GATE_REPORT}_20260704.md`.
