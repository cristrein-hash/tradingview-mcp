...
## Phase 2B.5 implemented: cross-specialist confluence audit (DIAGNÓSTICO) — 2026-06-19
**Outcome só pós-hoc. SEM aggregator/decisão/regra.** Responde: especialistas fracos sozinhos agregam em confluência? CSVs: `l2_bpt_specialist_{state_matrix,pairwise_confluence,three_way_confluence,confluence_by_context,confluence_hit_rate_metrics,classification_after_confluence}.csv`. DA ac573cc2 (shuffle-null 2000-iter + context-matched 10k + drop-top2).
- **ACHADO GENUÍNO (único que sobrevive a TODOS os gates): `capitulation+rsi_momentum`.** family-wise p=0.014 (shuffle-null), context-matched p=0.0098 (sinal INDEPENDENTE do contexto, não é só re-selecionar bottom_reversal), drop2 +1.25. **Métrica honesta = hit-2R 65% vs base 32% (lift 2.0×, Wilson-lo 41%).** Dois especialistas fracos sozinhos viram decisive JUNTOS — a hipótese do bloco confirmada, mas só p/ este par.
- **CAVEAT central:** n=17, 4 no cap +3.9R → avgR +1.56 é ~40% cap-inflado (floor-2R → +1.11). **In-sample, NÃO validado OOS.** Afirmação defensável = hit-rate lift, não +1.56R expectancy.
- **Inflado/herdado (NÃO sobrevivem):** 3-way capit+bub+rsi (subset do capit+rsi, dentro do shuffle-null tail); nas+demand+rsi (p=0.045 borderline, n13); bubbles/rsi_momentum STANDALONE (rsi solo lift 1.10×, bubbles 1.01× = ruído — só importam DENTRO da célula capit+rsi); DAveto+demand-hostile (context-matched p=0.11 → sem sinal além do contexto, veto NÃO validado).
- **volume_vp: NOISY confirmado** (nenhuma confluência/contexto defensável; células n=1-3).
- **Por contexto:** bottom_reversal_capitulation é onde quase tudo é forte (rsi +1.60/n13, nas +1.41, bull_beta +1.34); capitulation forte em bear_bounce (+0.82/57% hit2R, n7) e mid_range (+0.94/n9) — útil como detector de fundo escondido nesses rótulos.
- **Classificação pós-confluência:** nas=DECISIVE_INDIVIDUAL; exhaustion_top=DECISIVE_INDIVIDUAL(winner-sel, cap caveat); capitulation+rsi=GENUINE_CONFLUENCE(único); demand_supply/risk_sl=SUPPORTING; bubbles/rsi/bull_beta=CONTEXT_ONLY (não independentes); volume_vp=NOISY; devils_advocate=VETO_ONLY (veto ainda não validado além do contexto).
- **Phase 2C (aggregator) autorizável? NÃO AINDA.** Só 1 confluência (capit+rsi) sobrevive, p=0.014 n=17 (4 cap-pinned). Aggregator sobre 1 célula de 17 = overfit. **PRÓXIMO: validar capit+rsi OOS como HIT-RATE target (split temporal/sub-janelas) ANTES de qualquer aggregator.** É calibração, não validação (regra 45-grupos).
